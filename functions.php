...
function getConnexion(){
	...
	return mysqli_connect("localhost","root","","base");
}
function getUsers(){
...
function getAllMessagesRecus($iduser){
	...
	$cnx=getConnexion();
	$req=mysqli_query($cnx,"select * from message where userreceive={$iduser} and receivedel=0");
	$tab=[];
...
$cnx=getConnexion();
	$req=mysqli_query($cnx,"select * from user where iduser={$iduser}");
	$tab=null;
	while($d=mysqli_fetch_array($req)){
